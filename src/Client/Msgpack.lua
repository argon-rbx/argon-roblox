...
local encoder_functions = {
	...
	['number'] = function(value)
		if value ~= value then
			return pack('B', 0xcb) .. pack('>d', 0x7ff8000000000000)
		elseif value == math.huge then
			return pack('B', 0xcb) .. pack('>d', 0x7ff0000000000000)
		elseif value == -math.huge then
			return pack('B', 0xcb) .. pack('>d', 0xfff0000000000000)
		end
		...
		if is_int(value) then
			...
			if unpack('f', pack('f', value)) == value then -- check if we can use float
	...
	['table'] = function(value)
		if is_array(value) then -- it seems to be a proper Lua array
			local elements = {}
...
local function decode_map(data, position, length)
	local elements, key, value = {}, nil, nil
...
local decoder_functions = {
	[0xc0] = function(_data, position)
		return nil, position
	end,
	[0xc2] = function(_data, position)
		return false, position
	end,
	[0xc3] = function(_data, position)
...
for i = 0x00, 0x7f do
	decoder_functions[i] = function(_data, position)
		return i, position
	end
end
for i = 0x80, 0x8f do
...
return {
	...
	-- decode one value
	decode = function(data: string, position: number?): any?
		local value, ok
